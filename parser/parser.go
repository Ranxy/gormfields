...
const (
	structComment     = "gormfields:query"
	structSkipComment = "gormfields:skip"
)
...
func (v vistor) Visit(n ast.Node) (w ast.Visitor) {
	switch n := n.(type) {
	case *ast.Package:
		return v
	case *ast.File:
...
func readAst(filename string) error {
